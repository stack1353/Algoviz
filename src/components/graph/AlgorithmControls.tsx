...
// //               weight = Math.max(1, Math.floor(aiEdge.weight));
// //             }
...
// // Increased canvas dimensions for better node placement
// const GRAPH_CANVAS_WIDTH = 800;
// const GRAPH_CANVAS_HEIGHT = 600;
// const NODE_RADIUS_PADDING = 50;  // Increased padding for better spacing
...
//         // Process nodes with coordinate validation and fixed positioning
//         aiOutput.nodes.forEach((aiNode: ExtractedNode) => {
//           // Validate coordinates are between 0-1
//           const validatedX = Math.max(0, Math.min(1, aiNode.x));
//           const validatedY = Math.max(0, Math.min(1, aiNode.y));
...
//             id: internalNodeId,
//             label: aiNode.label || `N${nodeIdCounter-1}`,
//             // Fixed upside-down problem by inverting y-axis
//             x: NODE_RADIUS_PADDING + validatedX * (GRAPH_CANVAS_WIDTH - 2 * NODE_RADIUS_PADDING),
//             // Corrected y-coordinate calculation with inversion
//             y: GRAPH_CANVAS_HEIGHT - NODE_RADIUS_PADDING - validatedY * (GRAPH_CANVAS_HEIGHT - 2 * NODE_RADIUS_PADDING),
//           });
//         });

//         // Process edges with enhanced validation
//         aiOutput.edges.forEach((aiEdge: ExtractedEdge) => {
//           const sourceInternalId = aiIdToInternalIdMap.get(aiEdge.sourceId);
//           const targetInternalId = aiIdToInternalIdMap.get(aiEdge.targetId);

//           if (sourceInternalId && targetInternalId) {
//             // Handle missing/incorrect weights more robustly
//             let weight = 1;
//             if (aiEdge.weight !== undefined && !isNaN(aiEdge.weight)) {
//               // Ensure weight is at least 1
//               weight = Math.max(1, Math.round(aiEdge.weight));
//             }
